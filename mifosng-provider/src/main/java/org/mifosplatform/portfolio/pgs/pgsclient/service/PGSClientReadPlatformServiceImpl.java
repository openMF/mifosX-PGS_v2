...
import org.mifosplatform.portfolio.pgs.pgsclient.data.PGSClientTimelineData;
import org.mifosplatform.portfolio.pgs.pgsclient.domain.PGSClientEnumerations;
...
@Service
public class PGSClientReadPlatformServiceImpl implements PGSClientReadPlatformService {
    ...
    // data mappers
    private final PaginationHelper<PGSClientData> paginationHelper = new PaginationHelper<PGSClientData>();
    private final PGSClientMapper clientMapper = new PGSClientMapper();
    private final PGSClientLookupMapper lookupMapper = new PGSClientLookupMapper();
    private final PGSClientMembersOfGroupMapper membersOfGroupMapper = new PGSClientMembersOfGroupMapper();
    private final ParentGroupsMapper clientGroupsMapper = new ParentGroupsMapper();

    @Autowired
    public PGSClientReadPlatformServiceImpl(final PlatformSecurityContext context, final RoutingDataSource dataSource,
    ...
    @Override
    public PGSClientData retrieveTemplate(final Long officeId, final boolean staffInSelectedOfficeOnly) {
        ...
        final List<CodeValueData> genderOptions = new ArrayList<CodeValueData>(
                this.codeValueReadPlatformService.retrieveCodeValuesByCode(PGSClientApiConstants.GENDER));

        return PGSClientData.template(defaultOfficeId, new LocalDate(), offices, staffOptions, null, genderOptions, savingsProductDatas);
    }

    @Override
    public Page<PGSClientData> retrieveAll(final SearchParameters searchParameters) {
    ...
    @Override
    public PGSClientData retrieveOne(final Long clientId) {
        try {
            ...
            final PGSClientData clientData = this.jdbcTemplate.queryForObject(sql, this.clientMapper, new Object[] { hierarchySearchString,
            ...
            return PGSClientData.setParentGroups(clientData, parentGroups);
    ...
    @Override
    public Collection<PGSClientData> retrieveAllForLookup(final String extraCriteria) {
    ...
    @Override
    public Collection<PGSClientData> retrieveAllForLookupByOfficeId(final Long officeId) {

        final String sql = "select " + this.lookupMapper.schema() + " where c.office_id = ? and c.status_enum != ?";

        return this.jdbcTemplate.query(sql, this.lookupMapper, new Object[] { officeId, PGSClientStatus.CLOSED.getValue() });
    }

    @Override
    public Collection<PGSClientData> retrieveClientMembersOfGroup(final Long groupId) {
    ...
    @Override
    public Collection<PGSClientData> retrieveActiveClientMembersOfGroup(final Long groupId) {
        ...
        return this.jdbcTemplate.query(sql, this.membersOfGroupMapper,
                new Object[] { hierarchySearchString, groupId, PGSClientStatus.ACTIVE.getValue() });
    }

    private static final class PGSClientMembersOfGroupMapper implements RowMapper<PGSClientData> {

        private final String schema;

        public PGSClientMembersOfGroupMapper() {
            final StringBuilder sqlBuilder = new StringBuilder(200);

            sqlBuilder.append("c.id as id, c.pgs_client_account_no as accountNo, c.external_id as externalId, c.status_enum as statusEnum, ");
            ...
            sqlBuilder.append("c.mifos_client_id as mifosClientId, "); 
            sqlBuilder.append("c.service_account as serviceAccountId, ");
            ...
            sqlBuilder.append("from m_pgsclient c ");
        ...
        @Override
        public PGSClientData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {

            final String accountNo = rs.getString("accountNo");

            final Integer statusEnum = JdbcSupport.getInteger(rs, "statusEnum");
            final EnumOptionData status = PGSClientEnumerations.status(statusEnum);
            ...
            final Long mifosClientId = JdbcSupport.getLong(rs, "mifosClientId");
            final Long serviceAccountId = JdbcSupport.getLong(rs, "serviceAccountId");
            ...
            final PGSClientTimelineData timeline = new PGSClientTimelineData(submittedOnDate, submittedByUsername, submittedByFirstname,
                    submittedByLastname, activationDate, activatedByUsername, activatedByFirstname, activatedByLastname, closedOnDate,
                    closedByUsername, closedByFirstname, closedByLastname);

            return PGSClientData.instance(accountNo, status, officeId, officeName, transferToOfficeId, transferToOfficeName, id, firstname,
                    middlename, lastname, fullname, displayName, externalId, mobileNo, dateOfBirth, gender, activationDate, imageId,
                    staffId, staffName, timeline, savingsProductId, savingsProductName, savingsAccountId, mifosClientId, serviceAccountId);

        }
    }

    @Override
    public Collection<PGSClientData> retrieveActiveClientMembersOfCenter(final Long centerId) {
        ...
        return this.jdbcTemplate.query(sql, this.membersOfGroupMapper,
                new Object[] { hierarchySearchString, centerId, PGSClientStatus.ACTIVE.getValue() });
    }

    private static final class PGSClientMapper implements RowMapper<PGSClientData> {

        private final String schema;

        public PGSClientMapper() {
            final StringBuilder builder = new StringBuilder(400);

            builder.append("c.id as id, c.pgs_client_account_no as accountNo, c.external_id as externalId, c.status_enum as statusEnum, ");
            ...
            builder.append("c.mifos_client_id as mifosClientId, ");
            builder.append("c.service_account as serviceAccountId ");
            builder.append("from m_pgsclient c ");
    ...
    private static final class ParentGroupsMapper implements RowMapper<GroupGeneralData> {
        ...
        public String parentGroupsSchema() {
            return "gp.id As groupId , gp.display_name As groupName from m_pgsclient cl JOIN m_group_client gc ON cl.id = gc.client_id "
    ...
    private static final class PGSClientLookupMapper implements RowMapper<PGSClientData> {

        private final String schema;

        public PGSClientLookupMapper() {
        ...
        public PGSClientData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            return PGSClientData.lookup(id, displayName, officeId, officeName);
        }
    }

    @Override
    public PGSClientData retrieveClientByIdentifier(final Long identifierTypeId, final String identifierKey) {
        try {
            final PGSClientIdentifierMapper mapper = new PGSClientIdentifierMapper();
    ...
    private static final class PGSClientIdentifierMapper implements RowMapper<PGSClientData> {

        public String clientLookupByIdentifierSchema() {
            return "c.id as id, c.pgs_client_account_no as accountNo, c.status_enum as statusEnum, c.firstname as firstname, c.middlename as middlename, c.lastname as lastname, "
                    + "c.fullname as fullname, c.display_name as displayName,"
                    + "c.office_id as officeId, o.name as officeName "
                    + " from m_pgsclient c, m_office o, m_pgsclient_identifier ci "
        ...
        public PGSClientData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            return PGSClientData.clientIdentifier(id, accountNo, status, firstname, middlename, lastname, fullname, displayName, officeId,
                    officeName);
        }
    }
    ...
    @Override
    public PGSClientData retrieveAllClosureReasons(final String clientClosureReason) {
        ...
        return PGSClientData.template(null, null, null, null, closureReasons, null, null);
    }
